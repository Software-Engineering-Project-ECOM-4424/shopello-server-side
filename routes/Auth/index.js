...
router.post('adminlogin',
    ...
    async (req, res) => {
        ...
        try {
            ...
            const compared = await comparePasswords(rows[0].password, password);
            if (compared) {
                const { SECRET_KEY } = process.env;
                const userId = rows[0].id;
                const token = await signToken(userId, SECRET_KEY);
...
router.get('/product-details',
    async (req, res) => {
        try {
            const { rows } = await dbContext.query('SELECT id,name,description,price, image, category_id FROM products WHERE id = $1', [req.headers.product_id])
            if (rows.length > 0) {
                return res.status(200).json(rows);
            }
